...
│   │   ├── metrics.go
│   │   └── metrics_test.go
│   ├── network/               # LibP2P node + mDNS discovery
│   ├── search/                # LSH + Linear search para match de padrões
│   ├── trainer/               # Engine de treinamento (frequency + elite selection)
│   └── vfs/                   # FUSE mount + RandomReader com BlockCache
├── pkg/
│   ├── cromlib/               # Pack() e Unpack() — motor principal
│   │   ├── compiler.go        #   Loop de compressão (goroutines paralelas)
...
│   │   ├── format.go          #   Structs, Parse, Serialize
│   │   └── reader.go          #   Streaming reader (ReadMetadata)
│   ├── sdk/                   # SDK público (thin wrapper)
│   └── sync/                  # Sync primitives
├── pesquisa/                  # 6 pesquisas com datasets e scripts
...
| 06 — Imagens | 7 formatos, 1223 testes | **1121/1223 PASS (91.7%)** |
...
## Problemas Conhecidos Descobertos
1. **Codebook não generaliza** — degradação de 32.71% em dados novos (memorizer, não generalizer)
