#!/bin/bash
# ==============================================================
# APP DOCKER VFS: Docker via FUSE CASCADING
# Camadas: CROM Mount -> SquashFuse -> OverlayFs
# ==============================================================

APP_NAME="app_docker_vfs"
...
if ! command -v docker &>/dev/null; then log "${RED}❌ Docker não encontrado${NC}"; exit 1; fi
if [ ! -x "$CROM_BIN" ]; then log "${RED}❌ Motor CROM não encontrado!${NC}"; exit 1; fi

# ── FASE 1: GERAÇÃO CROM (MONÓLITO) ──
if [ ! -f "$CROM_FILE" ]; then
    log "  📦 [Camada 1] Escrevendo SquashFS Docker Base..."
    mkdir -p _db_base/src && echo "console.log('Original App');" > _db_base/src/app.js 
    dd if=/dev/urandom of=_db_base/sample.bin bs=1M count=15 status=none
...
mkdir -p "$MERGED_DIR/src"
echo "console.log('App Dockerizada via CROM VFS Overlay: v2.0.0');" > "$MERGED_DIR/src/app.js"
cat << 'EOF' > "$MERGED_DIR/Dockerfile"
FROM node:22-alpine
WORKDIR /app
COPY src/ /app/src/
RUN md5sum src/app.js > checksum.txt
CMD ["node", "src/app.js"]
EOF

log ""
log "${BOLD}🧪 Teste: Docker Build LENDO de 3 Camadas FUSE...${NC}"
START=$(date +%s%N)

docker build -t crom-vfs-test "$MERGED_DIR" 2>&1 | tee -a "$LOG_FILE"
EXIT_CODE=${PIPESTATUS[0]}

log "  Resultado do container rodando:"
docker run --rm crom-vfs-test 2>&1 | tee -a "$LOG_FILE"
RUN_EXIT=${PIPESTATUS[0]}

END=$(date +%s%N); ELAPSED=$(( (END - START) / 1000000 ))

if [ $EXIT_CODE -eq 0 ] && [ $RUN_EXIT -eq 0 ]; then
    log "${GREEN}✅ RESULTADO: PASS — Motor CROM aguentou leitura Docker CLI (${ELAPSED}ms)${NC}"
else
    log "${RED}❌ RESULTADO: FAIL (${ELAPSED}ms)${NC}"
fi

log "🔒 Desmontando VFS Cascading..."
docker rmi crom-vfs-test 2>/dev/null || true
fusermount -uz "$MERGED_DIR" 2>/dev/null || true
fusermount -uz "$MNT_RO" 2>/dev/null || true
fusermount -uz "$MNT_CROM" 2>/dev/null || true
