...
func daemonCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "daemon",
		Short: "Inicia o nó P2P do CROM para sincronização na rede soberana",
		Long: `Inicia um nó libp2p que participa da rede CROM definida pelo Codebook informado.
O nó irá descobrir outros peers via mDNS (LAN) e DHT (WAN), e permitirá a sincronização
de arquivos .crom na pasta data-dir.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			go func() {
				time.Sleep(2 * time.Second) // wait for pubsub to settle
...
func shareCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "share <file.crom>",
		Short: "Anuncia um arquivo .crom na rede soberana P2P",
...
func mountCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "mount",
		Short: "Monta um arquivo .crom como um VFS (Virtual Filesystem)",
		Long:  `Monta o arquivo compactado em um diretório vazio para leitura sob demanda (Lazy Loading) do conteúdo original.`,
		RunE: func(cmd *cobra.Command, args []string) error {
